...
TEST_F(Copa2Test, PersistentCongestion) {
  ...
  copa2.onPacketAckOrLoss(std::nullopt, loss);
  EXPECT_EQ(
...
TEST_F(Copa2Test, TestBwEstimate) {
  ...
  conn.lossState.lrtt = 100ms;
  now += 100ms;
  copa2.onPacketAckOrLoss(createAckEvent(5, 5 * packetSize, now), std::nullopt);
  ...
  copa2.onPacketAckOrLoss(createAckEvent(1, packetSize, now), std::nullopt);
  EXPECT_EQ(
...
TEST_F(Copa2Test, PacketLossInvokesPacer) {
  ...
  copa2.onPacketAckOrLoss(std::nullopt, lossEvent);
  // Ack one packet to test how we set pacing rate
  copa2.onPacketSent(createPacket(1, 1000, 2000));
  copa2.onPacketAckOrLoss(createAckEvent(1, 1000, Clock::now()), std::nullopt);
}
