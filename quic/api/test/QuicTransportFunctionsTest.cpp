...
TEST_F(QuicTransportFunctionsTest, EgressPolicerDropsPacketsChainedMemory) {
  ...
  // Set a very low rate: 1 byte/sec with burst of 1500 bytes.
  // First packet should pass (burst available), subsequent packets should drop.
  conn->egressPolicer = std::make_unique<folly::TokenBucket>(1.0, 1500.0);
  ...
  auto stream = conn->streamManager->createNextBidirectionalStream().value();
  // Write enough data for multiple packets.
  auto buf = IOBuf::copyBuffer(std::string(4000, 'x'));
  ASSERT_FALSE(writeDataToQuicStream(*stream, buf->clone(), true).hasError());

  // With 1 byte/sec and 1500 burst, first packet passes, rest should be
  // dropped. Socket write should only be called for the first packet.
...
TEST_F(QuicTransportFunctionsTest, EgressPolicerActivationDelay) {
  auto conn = createConn();
  // Set policer with 1 byte/sec (would drop everything after burst) but
