...
quic::Expected<void, QuicError> FizzOpenSSLAes128PacketNumberCipher::setKey(
    ByteRange key) {
  pnKey_ = BufHelpers::copyBuffer(key);
  return setKeyImpl(encryptCtx_, EVP_aes_128_ecb(), key);
}

quic::Expected<void, QuicError> FizzOpenSSLAes256PacketNumberCipher::setKey(
    ByteRange key) {
  pnKey_ = BufHelpers::copyBuffer(key);
  return setKeyImpl(encryptCtx_, EVP_aes_256_ecb(), key);
}

const BufPtr& FizzOpenSSLAes128PacketNumberCipher::getKey() const {
  return pnKey_;
}

const BufPtr& FizzOpenSSLAes256PacketNumberCipher::getKey() const {
  return pnKey_;
}

quic::Expected<HeaderProtectionMask, QuicError>
FizzOpenSSLAes128PacketNumberCipher::mask(ByteRange sample) const {
  return maskImpl(encryptCtx_, sample);
}

quic::Expected<HeaderProtectionMask, QuicError>
FizzOpenSSLAes256PacketNumberCipher::mask(ByteRange sample) const {
  return maskImpl(encryptCtx_, sample);
}

constexpr size_t kAES128KeyLength = 16;

size_t FizzOpenSSLAes128PacketNumberCipher::keyLength() const {
  return kAES128KeyLength;
}

constexpr size_t kAES256KeyLength = 32;

size_t FizzOpenSSLAes256PacketNumberCipher::keyLength() const {
